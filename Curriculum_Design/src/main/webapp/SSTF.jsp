...
<head>
  ...
  <script>
    function toindex() {
      window.location.href = "http://localhost:8080/Curriculum_Design/index.jsp";
    }
  </script>
</head>
...
<div class="container">
  ...
  <div class="top-left">
    ...
    <div class="head2">
      <form action="SSTFServlet" method="post" style="margin-left: 10px">
        <input type="submit" value="初始化" name="s3"
               class="button-intial">
      </form>

      <form action="SSTFServlet"  method="post" style="margin-left: 20px">
        <strong   style="color:black">磁头初始位置:</strong>
        <input type="number" min="0" step="1" name="firstNum" value="${firstNum}">

        <strong   style="color:black">分块:</strong>
        <input type="number" name="fenkuai" value="${fenkuai}">

        <input type="submit" value="确认" name="s4" class="button-intial" style="float:right">
    ...
    <div class="head3">
      ...
      <form action="SSTFServlet" method="post" style="margin-left: 118px">
  ...
  <button class="tbox3" onclick="toindex()" style=" font-family: Helvetica Neue, Arial, sans-serif; font-size: 20px; position: absolute; text-align: center; width: 20px; height: 20px; border-radius: 50%; background-color:  orange; display: flex; align-items: center; justify-content: center; font-weight: bold; ">
